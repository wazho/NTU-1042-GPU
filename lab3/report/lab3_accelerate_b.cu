...
__global__ void SuccessiveOverRelaxationMethod(const float times, const float *fixed, const float *mask, const float *target, float *output, const int wt, const int ht);
...
void PoissonImageCloning(const float *background, const float *target, const float *mask, float *output, const int wb, const int hb, const int wt, const int ht, const int oy, const int ox) {
	...
	for (int i = 0; i < ITERATION_TIMES; i++) {
		SuccessiveOverRelaxationMethod <<<gdim, bdim>>> (i, fixed, mask, buf1, buf2, wt, ht);
...
__global__ void SuccessiveOverRelaxationMethod(const float times, const float *fixed, const float *mask, const float *target, float *output, const int wt, const int ht) {
	...
	float w = 1.0 + (ITERATION_TIMES - times) / ITERATION_TIMES;
